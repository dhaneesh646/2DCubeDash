...
public class ParallelWorldWarning : MonoBehaviour
{
    ...
    void CheckForHazards()
    {
        ...
        if (hazardNearby)
        {
            float dangerLevel = CalculateDangerLevel(closestDistance);
            
            if (!isWarningActive)
            {
                isWarningActive = true;
                if (warningCoroutine != null) StopCoroutine(warningCoroutine);
                warningCoroutine = StartCoroutine(PulseWarning(dangerLevel));
            }
            else
            {
                // Update the danger level for the existing coroutine
                // This is handled within the coroutine itself
            }
        }
    ...
    IEnumerator PulseWarning(float baseIntensity)
    {
        ...
        while (isWarningActive)
        {
            ...
            float currentDanger = CalculateDangerLevel(closestDistance);
            
            // Calculate pulse parameters based on danger level
            float pulseRate = Mathf.Lerp(basePulseSpeed, basePulseSpeed * 3f, currentDanger); // Faster pulse for more danger
            float intensity = Mathf.Lerp(0.3f, maxPulseIntensity, currentDanger);
            
            // Animate one complete heart beat
            yield return StartCoroutine(AnimateHeartBeat(pulseRate, intensity, currentDanger));
        }
    }
